...
fetch('http://127.0.0.1:5000/data/customers', {
  ...
  .then( response => {
    ...
    users.forEach(user =>{
      ...
      upusername.setAttribute('onkeypress','return /[a-zA-Z_]/.test(String.fromCharCode(event.keyCode))')
...
let adminId = localStorage.getItem('idadmin');

let formDaata = {
  "id": adminId,
 }

fetch('http://127.0.0.1:5000/data/useradmin', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
  ...
  .then( response => {
    ...
    admins.forEach(admin =>{
      ...
      liadmin.append(idadmin,upadmin,upusername,passadmin)
